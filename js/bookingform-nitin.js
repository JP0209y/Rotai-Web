...
function selectLocation(el, e) {
  ...
  document.getElementById('input-location').value = location; // ✅ ADD
...
function selectProduct(el, e) {
  ...
  document.getElementById('input-product').value = product; // ✅ ADD
...
function selectTime(el, e) {
  ...
  document.getElementById('input-time').value = time; // ✅ ADD
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 function updateDateUI(date) {
  ...
  document.getElementById('input-date').value =
    date.toISOString().split('T')[0]; // ✅ ADD
}
